...
function Invoke-ScaffoldFiles {
  ...
  $workflowTpl = Join-Path $tplRoot 'workflow/deploy-android.yml.tpl'
  ...
  $fastlaneFiles = @(
    @{ Src = 'fastlane/Appfile.tpl';  Dst = 'android/fastlane/Appfile';  IsTemplate = $true }
    @{ Src = 'fastlane/Fastfile.tpl'; Dst = 'android/fastlane/Fastfile'; IsTemplate = $true }
    @{ Src = 'fastlane/Gemfile';      Dst = 'android/Gemfile';           IsTemplate = $false }
  )
